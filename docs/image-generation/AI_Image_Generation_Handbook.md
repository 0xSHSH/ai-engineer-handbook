...
```mermaid
flowchart LR
    XT[Pure Noise] -->|"Predict ε_T"| XT1["Less Noisy (t-1)"]
    XT1 -->|"Predict ε_T-1"| XT2["..."]
    XT2 -->|"Predict ε_1"| X0[Clean Latent]
```
...
flowchart TD
    ...
    Q --> WK["GPU Workers (Docker / Kubernetes)"]
    WK --> M["Diffusion Model (Stable Diffusion / FLUX)"]
